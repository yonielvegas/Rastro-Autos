...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    break;
        case 'actualizar':
    error_log("procesar_inventario.php: Acción recibida - actualizar.");
    ...
    // Obtener las rutas de imagen actuales de la base de datos
    $parte_existente = $inventario->obtenerPartePorId($id_parte);
    $imagen_original_path_db = $parte_existente['imagen'] ?? null;
    $imagen_thumbnail_path_db = $parte_existente['imagen_thumbnail'] ?? null;

    // Proceso de subida de nueva imagen
    if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
        $file = $_FILES['imagen'];
        ...
        if (move_uploaded_file($file['tmp_name'], $targetFilePathOriginal)) {
            // Eliminar imágenes antiguas si existen y se sube una nueva
            if ($parte_existente['imagen'] && file_exists('../inventario/' . $parte_existente['imagen'])) {
                unlink('../inventario/' . $parte_existente['imagen']);
            }
            if ($parte_existente['imagen_thumbnail'] && file_exists('../inventario/' . $parte_existente['imagen_thumbnail'])) {
                unlink('../inventario/' . $parte_existente['imagen_thumbnail']);
            }
            ...
            $response['message'] = 'Error al subir la nueva imagen principal.';
            echo json_encode($response);
            exit();
        }
    } else if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] !== UPLOAD_ERR_NO_FILE) {
        $response['message'] = 'Error en la subida del nuevo archivo. Código de error de PHP: ' . $_FILES['imagen']['error'];
        echo json_encode($response);
        exit();
    }
    // Si no se sube una nueva imagen, las rutas $imagen_original_path_db y $imagen_thumbnail_path_db mantienen los valores existentes.

    if ($inventario->actualizarParte($id_parte, $nombre, $descripcion, $precio, $cantidad_stock, $codigo_serie, $id_marca, $id_modelo, $id_cat, $imagen_original_path_db, $imagen_thumbnail_path_db, $fecha_registro)) {
        $response['success'] = true;
        $response['message'] = 'Parte actualizada exitosamente.';
    } else {
        $response['message'] = 'Error al actualizar la parte en la base de datos. Verifique logs.';
        // Si la actualización falla, y se había subido una nueva imagen, eliminarla
        if (isset($file) && file_exists($targetFilePathOriginal)) {
            unlink($targetFilePathOriginal);
        }
        if (isset($thumbnailer) && file_exists($targetFilePathThumbnail)) {
            unlink($targetFilePathThumbnail);
        }
    }
    break;
